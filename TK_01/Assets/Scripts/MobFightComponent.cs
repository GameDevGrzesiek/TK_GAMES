    ...
    void Update()
    {
        ...
        switch (m_state)
        {
            case MobState.ToTarget:
            {
                ...
                if (Physics.Raycast(curPos, dir, SceneSettings.ShootingRange, 1 << LayerMask.NameToLayer("Wall")))
            ...
            case MobState.Throw:
            {
                SpearBehavior spear = PoolManager.Instance.SpearPool.SpawnObject(curPos + SceneSettings.ThrowingPoint, 
                                                                                 Quaternion.Euler(SceneSettings.ThrowingRotation)) as SpearBehavior;
