...
public class UIManager : Singleton<UIManager>
{
    ...
    public void RefreshPoolCount()
    {
        ...
        MobCountLabel.text = "Mob Count: " + GameManager.Instance.MobCnt.ToString();
    }

    public void ChangeMobCnt(int changeAmount)
    {
        if (changeAmount > 0)
            GameManager.Instance.SpawnMobs(changeAmount);
        else
            GameManager.Instance.DespawnMobs(changeAmount);
    }
}
